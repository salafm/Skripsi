...
class ARView: UIViewController {
    ...
    @IBAction func exitAR(_ sender: UIButton) {
...
extension ARView : CAAnimationDelegate {
...
extension ARView {
    ...
    func session(_ session: ARSession, didFailWithError error: Error) {
        ...
        topVC?.present(tabBar, animated: true, completion: {() -> Void in
            self.showAlert(header: "Tidak Support", message: "Perangkat anda tidak mendukung fitur augmented reailty. Fitur ini tersedia pada iphone 6s ke atas.")
        })
    }
        ...
        if let worldSectionsPositions = worldSectionsPositions {
            ...
            if let carLocation = carLocation, let sectionCoordinates = sectionCoordinates , let firstSection = sectionCoordinates.first, firstSection.count > 0 {
                if let referencePoint = firstSection.first {
                    ...
                    let node = SceneNodeCreator.createNodeWithImage(image: UIImage(named: "aura")!, position: position, width: 10, height: 10)
